...
parser.add_argument("-u",
                    "--update",
                    action="store_true",
                    help="Updates reference files to match the new outputs")
...
def utils_test(input_fn: str | None = None,
               init_args: list[str] = None,
               update_args: list[str] = None,
               info_args: list[str] = None,
               expect_success: bool = True,
               expected_info_fn: str | None = None,
               expected_data_fn: str | None = None):
    all_args = locals()
    ...
    def run_util(util: str, args: list[str] = [], input=None):
        ...
        proc = subprocess.run(args=proc_args,
                              input=input,
                              capture_output=True,
                              check=False,
                              cwd=tests_dir)
    ...
    try:
        ...
        if update_args is not None:
            binary_output = run_util("rec_update",
                                     args=update_args,
                                     input=binary_output)

        if expected_data_fn is not None:
            print(
                f"  Comparing rec_ibinary datanfo against '{expected_data_fn}'"
            )
...
for file in tests_dir.glob("validate/*.yaml"):
    utils_test(
        init_args=[
            "--size=312", "--aux-region=32",
            "--ndef-uri=https://3dtag.org/s/334c54f088"
        ],
        update_args=[str(file)],
        info_args=[
            "--validate", "--extra-required-fields=sample_requirements.yaml"
        ],
    )
...
if True:
    utils_test(
        init_args=["--size=312", "--aux-region=32"],
        update_args=[
            f"{tests_dir}/specific/unknown_data.yaml",
            f"--config-file={tests_dir}/specific/unknown_config.yaml"
        ],

...
env.globals["fields_table"] = lambda file, category=None: generate_table(
    f"{data_dir}/{file}.yaml", cbor_region_columns, lambda row:
    (category is None) or row.get("category", "") == category)
env.globals["enum_table"] = lambda file, columns=enum_columns: generate_table(
    f"{data_dir}/{file}.yaml", columns)
...
class PythonCodeExtension(jinja2.ext.Extension):
    ...
    def parse(self, parser):
        ...
        return jinja2.nodes.CallBlock(self.call_method("_render"), [], [],
                                      body)
...
def show_example(prompt, language="yaml"):
    ...
    prompt = prompt.replace("--config-file=",
                            f"--config-file={os.path.abspath(data_dir)}/")
    prompt = "set -o pipefail; " + prompt
    output = subprocess.run(prompt,
                            shell=True,

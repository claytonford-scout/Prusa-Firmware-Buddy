...
def nfc_initialize(args: Args):
    ...
    assert (
        args.size % 8
    ) == 0, f"Tag size {args.size} must be divisible by 8 (to be encodable in the CC)"
    ...
    ndef_message_length = args.size - capability_container_size - len(
        tlv_terminator) - ndef_tlv_header_size
    ...
    metadata = dict()
    meta_fields = Fields.from_file(os.path.join(config_dir,
                                                config.meta_fields))
    ...
    if args.aux_region is not None:
        ...
        aux_region_offset = align_region_offset(payload_size - args.aux_region,
                                                align_up=False)
    ...
    meta_section_size = write_section(
        0,
        meta_fields.encode(metadata,
    ...
    if len(ndef_data) != expected_size:
        sys.exit(
            f"NDEF record calculated incorrectly: expected size {expected_size} ({preceding_records_size} + {ndef_header_size} + {payload_size}), but got {len(ndef_data)}"
        )
    ...
    assert full_data[ndef_payload_start:ndef_payload_start +
...
if __name__ == "__main__":
    parser = simple_parsing.ArgumentParser(
        prog="nfc_initialize",
        description=
        "Initializes an 'empty' (with no static or aux data) NFC tag to be used as a Prusa Material tag.\nThe resulting bytes to be written on the tag are returned to stdout.",
    )

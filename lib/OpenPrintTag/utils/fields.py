...
class NumberField(Field):
    ...
    def decode(self, data):
        num = float(data)
        return int(num) if num.is_integer() else round(
            num, CompactFloat.decimal_precision)
...
class EnumField(Field):
    ...
    def __init__(self, config, config_dir):
        ...
        items = yaml.safe_load(
            open(os.path.join(config_dir, config["items_file"]), "r"))
        for item in items:
...
class Fields:
    ...
    def from_file(file: str):
        r = Fields()
        r.init_from_yaml(yaml.safe_load(open(file, "r")),
    ...
    def decode(self,
               binary_data: typing.IO[bytes],
               out_unknown_fields: dict[any, any] = None):
    ...
    def encode(
        self, data: dict[str,
                         any], config: EncodeConfig = EncodeConfig()) -> bytes:
    ...
    def validate(self, decoded_data):
        for field_name, field in self.fields_by_name.items():
            ...
            match field.required:
                ...
                case "recommended":
                    print(f"Missing recommended field '{field.name}'",
                          file=sys.stderr)

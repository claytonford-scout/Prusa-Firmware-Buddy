...
class Region:
    ...
    def __init__(self, record, offset: int, memory: memoryview,
                 fields: Fields):
        assert type(memory) is memoryview
        assert len(
            memory
        ) <= 512, "Specification prohibits memory regions larger than 512 bytes"
    ...
    def read(self,
             out_unknown_fields: dict[any, any] = None) -> dict[str, any]:
        if self.is_corrupt:
            return {}

        return self.fields.decode(io.BytesIO(self.memory),
                                  out_unknown_fields=out_unknown_fields)
    ...
    def update(self,
        ...
        assert encoded_len <= len(
            self.memory
        ), f"Data of size {encoded_len} does not fit into region of size {len(self.memory)}"
    ...
    def __init__(self, config_file: str, data: memoryview):
        ...
        match self.config.root:
            ...
            case "nfcv":
                ...
                assert cc[
                    0] == 0xE1, "Capability container magic number does not match"
                ...
                for record in ndef.message_decoder(data_io):
                    ...
                    raise Exception(
                        f"Did not find a record of type '{self.config.mime_type}'"
                    )
    ...
    def _setup_regions(self):
        if "meta_fields" not in self.config.__dict__:
            ...
            self.main_region = Region(
                0, self.payload,
                Fields.from_file(
                    os.path.join(self.config_dir, self.config.main_fields)))
        ...
        metadata = Region(
            self, 0, self.payload[0:meta_section_size],
            Fields.from_file(
                os.path.join(self.config_dir,
                             self.config.meta_fields))).read()

        main_region_offset = metadata.get("main_region_offset",
                                          meta_section_size)
        ...
        assert (not has_aux_region) or (
            aux_region_size
            is None), "aux_region_size present without aux_region_offset"

        region_stops = list(
            filter(lambda x: x is not None,
                   [main_region_offset, aux_region_offset,
                    len(self.payload)]))
        region_stops.sort()

        def create_region(offset, size, fields):
            if size is None:
                size = list(filter(lambda a: a > offset,
                                   region_stops))[0] - offset

            result = Region(
                self, offset, self.payload[offset:offset + size],
                Fields.from_file(os.path.join(self.config_dir, fields)))
        ...
        self.main_region = create_region(main_region_offset, main_region_size,
                                         self.config.main_fields)
        self.regions = {"meta": self.meta_region, "main": self.main_region}

        if has_aux_region:
            self.aux_region = create_region(aux_region_offset, aux_region_size,
                                            self.config.aux_fields)

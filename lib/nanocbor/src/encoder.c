...
static void _encoder_mem_append(nanocbor_encoder_t *enc, void *ctx,
...
static inline void _append(nanocbor_encoder_t *enc, const uint8_t *data,
                           size_t len)
{
...
static int _fmt_halffloat(nanocbor_encoder_t *enc, uint16_t half)
{
    ...
    if (res > 0) {
        uint8_t tmp[3] = { NANOCBOR_MASK_FLOAT | NANOCBOR_SIZE_SHORT,
                           (half >> HALF_SIZE / 2), half & HALF_MASK_HALF };

...
void create_url_string(std::array<char, url_buff_size> &url_buff, std::array<char, url_buff_size> &escaped_url_string) {
    [[maybe_unused]] auto rc = snprintf(url_buff.data(), url_buff.size(), "/?printer_type=%s&version=%s&mac=%s", PrinterModelInfo::current().id_str, version::project_version_full, otp_get_mac_address_str().data());
